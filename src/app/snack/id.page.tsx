...
const IdPage = () => {
  ...
  const snackQuery = useQuery({
    queryKey: ['snack', id],
    queryFn: async () => {
      const response = await getSnackById(id);
      return response.data;
    },
  });

  const data = snackQuery.data;
  ...
  const reviewsMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['snack', id] });
      reset();
    },
  });
  ...
  if (!snackQuery.isLoading && snackQuery.error) {
    return <Navigate to="/snack" />;
  }

  const ableToAddReview = !reviewsDisabled && isValid;
  const ableToAddToCart = isAuthenticated && !snackQuery.isLoading;
    ...
    <Container>
      <Stack direction="column" gap="2rem">
        {data ? (
          ...
          >
            <Box flex={3}>
              <img src={data.image_url} alt="Snack" width="100%" />
            </Box>
            <Box flex={4}>
...
const SnackInfoSkeleton = () => {
  ...
  return (
    <Stack
      ...
      direction="row"
      gap={theme => theme.spacing(5)}
      data-testid="snack-id-info-skeleton"
    >
      <Box flex={3}>
