...
const ListPage = () => {
  ...
  const snackQuery = useQuery({
    queryKey: ['page-snacks', page, sorting],
    refetchOnMount: true,
    queryFn: async () => {
      const response = await getPageSnacks(sorting, page, PAGE_SIZE);
  ...
  const totalPages = Math.ceil((snackQuery.data?.info.total || 0) / PAGE_SIZE);
    ...
    <Container>
      <Stack gap="1rem">
        ...
        <Masonry columns={4} spacing={2}>
          {snackQuery.data
            ? snackQuery.data.items.map(item => (
                <ProductCard
                  key={item.id}
                  title={item.name}
                  price={item.price}
